...
import org.example.mybatis.tkMapper.TkUserMapper;
...
public class TkMybatisTest {
    ...
    public void testInsert() {
        ...
        User testSave3 = User.builder().name("testSave3").password(SecureUtil.md5("123456") + salt).salt(salt).email("testSave3@xkcoding.com")
                .phoneNumber("17300000003").status(1).lastLoginTime(new DateTime()).createTime(new DateTime())
    ...
    public void testInsertList() {
        ...
        for (int i = 4; i < 14; i++) {
            String salt = IdUtil.fastSimpleUUID();
            User user = User.builder().name("testSave" + i).password(SecureUtil.md5("123456" + salt)).salt(salt)
                    .email("testSave" + i + "@xkcoding.com").phoneNumber("1730000000" + i).status(1)
    ...
    public void testQueryByCondition() {
        ...
        // 分页
        PageHelper.startPage(2, 3);//第2页的数据，每页3条
        // 查询
        List<User> userList = userMapper.selectByExample(example);
        PageInfo<User> userPageInfo = new PageInfo<>(userList);
        Assert.assertEquals(2, userPageInfo.getSize());
        Assert.assertEquals(count, userPageInfo.getTotal());
        log.debug("【userPageInfo】= {}", userPageInfo);
    }
